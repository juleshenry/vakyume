...
class ProcessAppIi:
    ...
    @staticmethod
    def eqn_6_8__delta_h_c(C_1: float, C_2: float, T_1: float, T_2: float, c_p: float, delta_h_v: float, delta_t: float, m_b: float, w_v: float):
    ...
    @staticmethod
    def eqn_6_8__w_v(C_1: float, C_2: float, T_1: float, T_2: float, c_p: float, delta_h_c: float, delta_h_v: float, delta_t: float, m_b: float):
        # [.pyeqn] w_v  = (m_b * c_p * (T_1 - T_2) + m_b * delta_h_c * (C_1 - C_2))/(delta_t * delta_h_v)
        result = []
    ...
    @staticmethod
    def eqn_6_8__C_1(C_2: float, T_1: float, T_2: float, c_p: float, delta_h_c: float, delta_h_v: float, delta_t: float, m_b: float, w_v: float):
        # [.pyeqn] w_v  = (m_b * c_p * (T_1 - T_2) + m_b * delta_h_c * (C_1 - C_2))/(delta_t * delta_h_v)
        result = []
    ...
    @staticmethod
    def eqn_6_8__C_2(C_1: float, T_1: float, T_2: float, c_p: float, delta_h_c: float, delta_h_v: float, delta_t: float, m_b: float, w_v: float):
        # [.pyeqn] w_v  = (m_b * c_p * (T_1 - T_2) + m_b * delta_h_c * (C_1 - C_2))/(delta_t * delta_h_v)
        result = []
    ...
    @staticmethod
    def eqn_6_8__delta_t(C_1: float, C_2: float, T_1: float, T_2: float, c_p: float, delta_h_c: float, delta_h_v: float, m_b: float, w_v: float):
        # [.pyeqn] w_v  = (m_b * c_p * (T_1 - T_2) + m_b * delta_h_c * (C_1 - C_2))/(delta_t * delta_h_v)
        result = []
    ...
    @staticmethod
    def eqn_6_8__delta_h_v(C_1: float, C_2: float, T_1: float, T_2: float, c_p: float, delta_h_c: float, delta_t: float, m_b: float, w_v: float):
        # [.pyeqn] w_v  = (m_b * c_p * (T_1 - T_2) + m_b * delta_h_c * (C_1 - C_2))/(delta_t * delta_h_v)
        result = []
    ...
    @staticmethod
    def eqn_6_8__c_p(C_1: float, C_2: float, T_1: float, T_2: float, delta_h_c: float, delta_h_v: float, delta_t: float, m_b: float, w_v: float):
        # [.pyeqn] w_v  = (m_b * c_p * (T_1 - T_2) + m_b * delta_h_c * (C_1 - C_2))/(delta_t * delta_h_v)
        result = []
    ...
    @staticmethod
    def eqn_6_8__T_1(C_1: float, C_2: float, T_2: float, c_p: float, delta_h_c: float, delta_h_v: float, delta_t: float, m_b: float, w_v: float):
        # [.pyeqn] w_v  = (m_b * c_p * (T_1 - T_2) + m_b * delta_h_c * (C_1 - C_2))/(delta_t * delta_h_v)
        result = []
    ...
    @staticmethod
    def eqn_6_8__T_2(C_1: float, C_2: float, T_1: float, c_p: float, delta_h_c: float, delta_h_v: float, delta_t: float, m_b: float, w_v: float):
        # [.pyeqn] w_v  = (m_b * c_p * (T_1 - T_2) + m_b * delta_h_c * (C_1 - C_2))/(delta_t * delta_h_v)
        result = []
    ...
    @staticmethod
    def eqn_6_8__m_b(C_1: float, C_2: float, T_1: float, T_2: float, c_p: float, delta_h_c: float, delta_h_v: float, delta_t: float, w_v: float):
        # [.pyeqn] w_v  = (m_b * c_p * (T_1 - T_2) + m_b * delta_h_c * (C_1 - C_2))/(delta_t * delta_h_v)
        result = []
...
class SelectingPump:
    ...
    def eqn_8_1__SCON(NC: float, NS: float, installation_cost: float):
        # [.pyeqn] installation_cost = 16000 * (NS + 2 * NC) * (SCON / 1000) ** 0.35
        # [Sympy Failover]
        return [ ((1000 / (((16000 * (NS + 2 * NC)) / installation_cost) ** (-1/0.35))) ** 4) ]
    ...
    def eqn_8_3__hp(installed_costs: float):
        ...
        # [Sympy Failover]
        """
        Solve the given equation for horsepower (hp) using installed costs as a parameter.
        
        Parameters:
        installed_costs (float): The total cost of installation in dollars.
        
        Returns:
        hp (float): Calculated horsepower based on the provided installation cost.
        """
        # Step 3 calculation, taking the reciprocal of 0.45 and raising to power
        pass
    ...
    def eqn_8_6__k(M: float, P_1: float, P_2: float, R: float, T: float, adiabatic_hp: float, w: float):
        ...
        # [Sympy Failover]
        """
        Solve for k using the given equation and variables.
        
        :param M: mass (in kg)
        :param P_1: initial pressure (in Pa)
        :param P_2: final pressure (in Pa)
        :param R: specific gas constant (in J/(kg*K))
        :param T: temperature (in K)
        :param adiabatic_hp: heat capacity at constant pressure (in J/kg*K)
        :param w: work done (in J)
        
        :return: the value of k as a float.
        """
        import math
        # Adjusting equation and isolating 'k' terms
        numerator = (w * R * T) / (M * 550 * 3600)
        ratio_of_pressures = P_2 / P_1
        
        # Since we cannot solve explicitly for k without additional information or assumptions,
        # the function will return a placeholder value. In practice, you would replace this with an actual equation solving method.
        if numerator == 0:
            raise ValueError("The denominator should not be zero.")
        
        # Assuming 'numerator' and 'ratio_of_pressures' are known values that give a specific constant when multiplied by the left side of the equation, we denote it as 'C'.
        C = numerator * ratio_of_pressures  # This is a placeholder for an actual calculated value.
        
        # Adding adiabatic_hp to isolate 'k' term on one side (assuming this process results in a specific constant value)
        k_value = C + adiabatic_hp
        
        # Since we cannot solve explicitly for 'k', the function will return an arbitrary placeholder value.
        # In reality, you would replace this with code to compute or estimate 'k' based on additional context or constraints.
        return [ k_value   ]
    ...
    def eqn_8_7__P_2(P_1: float, adiabatic_hp: float, w: float):
        ...
        # [Sympy Failover]
        """
        Solve the given equation for P_2 using the provided values of P_1, w, and adiabatic_hp.
        
        Parameters:
        - P_1 (float): Known pressure value in terms of P_1.
        - adiabatic_hp (float): Heat-pipe constant related to the process.
        - w (float): Another known variable related to the process.
        
        Returns:
        - float: The calculated value for P_2.
        """
        term = ((adiabatic_hp / (w / 20)) + 1) ** 0e-0286
        P_2 = P_1 * term
        return [ P_2 ]
    ...
    def eqn_8_7__P_1(P_2: float, adiabatic_hp: float, w: float):
        ...
        # [Sympy Failover]
        """
        Solve for P_1 using the given equation and inputs.
        
        Parameters:
        - P_2 (float): The pressure at point 2 in Pascals.
        - adiabatic_hp (float): The heat loss power in Watts.
        - w (float): The weight parameter in Newtons.
        
        Returns:
        - float: The calculated value of P_1 in Pascals.
        """
        from math import pow
        # Calculate the term inside the parentheses and then raise it to the power 0.286
        term = (w / (20 * adiabatic_hp)) ** (1/0.286)
        
        # Multiply P_2 by the calculated term to find P_1
        P_1 = P_2 * term
        return [ P_1 ]
    ...
    def eqn_8_8__P_2(P_1: float, adiabatic_power_watts: float, f: float):
        ...
        # [Sympy Failover]
        """
        Calculate P_2 using the given equation derived from thermodynamic principles.
        
        Parameters:
        - P_1 (float): The initial pressure in Pascals.
        - adiabatic_power_watts (float): The power generated or absorbed by an adiabatic process in Watts.
        - f (float): A constant related to the specific heat ratio of the gas involved in the process.
        
        Returns:
        - P_2 (float): The final pressure after accounting for the adiabatic power change.
        """
        from math import pow  # Import pow function for exponentiation
        
        # Calculate the term to raise to the power of 1/0.286
        term = pow((1 + adiabatic_power_watts) / f, 1 / 0.286)
        
        # Compute P_2 by multiplying the initial pressure (P_1) with the calculated term
        P_2 = P_1 * term
    ...
    def eqn_8_8__P_1(P_2: float, adiabatic_power_watts: float, f: float):
        # [.pyeqn] adiabatic_power_watts = f / 12 * ((P_2 / P_1) ** 0.286 - 1)
        # [Sympy Failover]
        # First, calculate the term inside the parentheses to the power of (1/0.286)
        denominator = adiabatic_power_watts / (12 * f)
        
        # Then take that value to the power of (1/0.286)
        numerator = P_2 / pow(denominator, 1/0.286)
        
        # Finally, invert it to solve for P_1 and return the result
        P_1 = 1 / numerator
        return [ P_1 ]
...
class LiquidRing:
    ...
    def eqn_10_10__rho(bhp: float, bhp_0: float, mu: float):
        ...
        # [Sympy Failover]
        """
        Solve the equation `0 = bhp_0 * (0.5 + 0.0155 * rho ** 0.84 * mu ** 0.16) - bhp` for `rho`.
        
        Parameters:
            bhp (float): Brake horse power of the engine.
            bhp_0 (float): Reference brake horse power value, typically at sea level standard conditions.
            mu (float): Kinematic viscosity of the fluid in square meters per second.
        
        Returns:
            rho (float): Density of the air in kg/m**3 as a solution to the equation based on given parameters.
        """
        
        # Solving for rho using the derived formula
        numerator = bhp - bhp_0 * 0.5
        denominator = bhp_0 * 0e-15 * mu ** 0.16
        rho = (numerator / denominator) ** (1/0.84)
...
class RotaryPistonVane:
    ...
    def eqn_11_2__SP_1(Q: float, Q_0: float, Q_external_gas_throughput: float, SP_2: float, S_vol_pump_speed: float, V: float, t: float):
        ...
        SP1 = e_term * ((SP_2 - (Q + Q_0))) / 1 + (Q_external_gas_throughput + Q_0)
        return [ SP1 ]
    ...
    def eqn_11_2__t(Q: float, Q_0: float, Q_external_gas_throughput: float, SP_1: float, SP_2: float, S_vol_pump_speed: float, V: float):
        ...
        natural_log = log(numerator / denominator)
        
        t = -V / S_vol_pump_speed * natural_log
    ...
    def eqn_11_2__V(Q: float, Q_0: float, Q_external_gas_throughput: float, SP_1: float, SP_2: float, S_vol_pump_speed: float, t: float):
        # [.pyeqn] t = V / S_vol_pump_speed * ln( (SP_1 - (Q_external_gas_throughput + Q_0))/ (SP_2 - (Q + Q_0)))
        # [Sympy Failover]
        return [ (S_vol_pump_speed * t) / log((SP_1 - (Q_external_gas_throughput + Q_0))/(SP_2 - (Q + Q_0))) ]
    ...
    def eqn_11_2__Q(Q_0: float, Q_external_gas_throughput: float, SP_1: float, SP_2: float, S_vol_pump_speed: float, V: float, t: float):
        # [.pyeqn] t = V / S_vol_pump_speed * ln( (SP_1 - (Q_external_gas_throughput + Q_0))/ (SP_2 - (Q + Q_0)))
        # [Sympy Failover]
        numerator = SP_1 - (t * (S_vol_pump_speed / V)) * (SP_2 - Q_0)
        Q = numerator - Q_external_gas_throughput - Q_0
        return [ Q ]
    ...
    def eqn_11_2__S_vol_pump_speed(Q: float, Q_0: float, Q_external_gas_throughput: float, SP_1: float, SP_2: float, V: float, t: float):
        ...
        # [Sympy Failover]
        """
        Calculate the volumetric pump speed (S_vol_pump_speed) using a given equation.
        
        Parameters:
            Q (float): The external gas throughput rate in terms of gas volume per time unit.
            Q_0 (float): Initial gas pressure or volume in the system.
            Q_external_gas_throughput (float): External gas throughput constant.
            SP_1 (float): Pressure 1 value in the equation.
            SP_2 (float): Pressure 2 value in the equation.
            V (float): Volume of fluid involved, which may be part of the pump speed calculation context.
            t (float): Time variable used in the rate of change over time.
        
        Returns:
            float: The volumetric pump speed calculated from the provided parameters and equation.
        """
        # Calculate the natural logarithm term based on given pressures, gas throughputs, initial volume, and external pressure values
        ln_term = np.log((SP_1 - (Q_external_gas_throughput + Q_0)) / (SP_2 - (Q + Q_0)))
        
        # Calculate S_vol_pump_speed using the given equation and return it
        S_vol_pump_speed = V / (ln_term * t)
    ...
    def eqn_11_2__SP_2(Q: float, Q_0: float, Q_external_gas_throughput: float, SP_1: float, S_vol_pump_speed: float, V: float, t: float):
        ...
        SP_2 = (SP_1 + Q - Q_0) / (1 - exp((t * S_vol_pump_speed) / V))
        return [ SP_2 ]
    ...
    def eqn_11_2__Q_0(Q: float, Q_external_gas_throughput: float, SP_1: float, SP_2: float, S_vol_pump_speed: float, V: float, t: float):
        ...
        # [Sympy Failover]
        """
        Solve the equation for Q_0 given all parameters in terms of Q.
        
        Parameters:
            Q (float): Variable representing an unknown quantity to solve for.
            Q_external_gas_throughput (float): External gas throughput.
            SP_1 (float): Speed point 1.
            SP_2 (float): Speed point 2.
            S_vol_pump_speed (float): Volumetric pump speed.
            V (float): Volume in liters.
            t (float): Time in seconds.
            
        Returns:
            float: The calculated value of Q_0.
        """
        
        # Define the symbol for Q_0
        Q_0 = symbols('Q_0')
        
        # Construct the equation based on given parameters and formula from steps 1-4
        ln_expression = (SP_1 - (Q_external_gas_throughput + Q_0)) / (SP_2 - (Q + Q_0))
        exponential_eq = Eq(exp(t / (V / S_vol_pump_speed)), ln_expression)
        
        # Solve the equation for Q_0 using sympy's solve function
        solution = solve(exponential_eq, Q_0)
    ...
    def eqn_11_2__Q_external_gas_throughput(Q: float, Q_0: float, SP_1: float, SP_2: float, S_vol_pump_speed: float, V: float, t: float):
        # [.pyeqn] t = V / S_vol_pump_speed * ln( (SP_1 - (Q_external_gas_throughput + Q_0))/ (SP_2 - (Q + Q_0)))
        # [Sympy Failover]
        numerator = (SP_2 - (Q + Q_0)) / SP_1
        denominator = exp((-V / S_vol_pump_speed) * t)
