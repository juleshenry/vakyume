...
#6-8
'''
Average vapor load to booster during time interval delta_t, lb/hr
w_v  = (m_b * c_p * (T_1 - T_2) + m_b * delta_h_c * (C_1 - C_2))/(delta_t * delta_h_v)
'''
#6-9
'''
dV / dt = (A * delta_P) / (mu * (m / A) * r * delta_P + r_M)
'''
#6-10
'''
dV / dt = (A * delta_P**(1 - s) ) / ( mu * tau * r_c )
'''
#6-11
'''
t_R = delta_h_i * m_b * delta_m / (A_d * h_d delta_T) = (delta_h_v * m_b * (m_i - m_l)) / (A_d * h_d * (T_w - T_s))
'''
