    ...
    def eqn_7_14b__del_T_1(
        ...
        def _res(del_T_1_val):
            try:
                ...
                sqrt = _math.sqrt
                exp = _math.exp
                val = (Q_condensor_heat_duty / (U * (del_T_1_val - del_T_2))) / log(
                    del_T_1_val - del_T_2
                ) - A
                # Extract real part; params may be complex from other shards
        ...
        param_vals = []
        for _pv in [A, Q_condensor_heat_duty, U, del_T_2]:
            try:
                _pv_f = float(_pv.real) if hasattr(_pv, "real") else float(_pv)
                if np.isfinite(_pv_f):
                    param_vals.append(_pv_f)
        ...
        if not np.isfinite(mid_r):
            ...
            for sub_pts in [np.linspace(lo, mid, 50), np.linspace(mid, hi, 50)]:
                ...
                if found_sub:
                    break
            if not found_sub:
                raise UnsolvedException("Bracket spans discontinuity for del_T_1")
    ...
    def eqn_7_14b__del_T_2(
        ...
        def _res(del_T_2_val):
            try:
                ...
                sqrt = _math.sqrt
                exp = _math.exp
                val = (Q_condensor_heat_duty / (U * (del_T_1 - del_T_2_val))) / log(
                    del_T_1 - del_T_2_val
                ) - A
                # Extract real part; params may be complex from other shards
                return val.real if hasattr(val, "real") else float(val)
        ...
        # Add parameter-based offsets to catch roots near param values
        param_vals = []
        for _pv in [A, Q_condensor_heat_duty, U, del_T_1]:
            try:
                _pv_f = float(_pv.real) if hasattr(_pv, "real") else float(_pv)
                if np.isfinite(_pv_f):
                    param_vals.append(_pv_f)
