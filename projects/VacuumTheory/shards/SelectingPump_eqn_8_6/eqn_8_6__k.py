...
def eqn_8_6__k(self, M: float, P_1: float, P_2: float, R: float, T: float, adiabatic_hp: float, w: float, **kwargs):
    ...
    def _res(k_val):
        try:
            ...
            val = (target_var_complex / (target_var_complex - 1) * (w * R * T) / (M * 550 * 3600) * ((P_2 / P_1) ** ((target_var_complex - 1) / target_var_complex) - 1)) - adiabatic_hp
