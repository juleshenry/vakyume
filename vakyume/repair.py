"""
                                                               :+ .x..                                                                        
                                                          .:...x$   $+:x  $.                                                                  
                                                      ..:. XX.  &    X.   xx$....                                                             
                                                      .:X . .             & . X.                                                              
                                                   .:;X+++xX&$X+:.   .    . .x  x..                                                           
                                                 .:$++++++++++x+......:;x$$X+..+..                         .                                  
                                               .:$x+x++++++++++x$...............&..                      .:&xxx&X.                            
                                              .:$++++++++++++++++$...:&$........$;:.                    .$xxX&XxxxxX+                         
                                              :$++++++++++++++++++X..&&$$X+....;&.;..                  :Xxx&$$$$$$$xxxxXX.                    
                                      ...    .:x++++++++++++++++++&:$&&XXX$..:&&$X$$.                 .+xxxx$$$$$$$$$X&$xx+xx.                
                                     .:$X$X..:X++++$+++x$+++++++++$::&&$XX$..x&$XXX.+.                 ;x$xxxxxx$$$$$$$$$$$&$x+$X.            
                                         .   :$++xX$$X++X+++++++++$:..:.::....X&X$x.$.                 :xxX$xxxxxxxx$&X$$$$$$$$$$xx;          
                                             .+++++Xx+xxx+++++++++X.......::........X.                 :XxxxX$xxxxxxxxxxX&$X$$$&+xX           
                                              .$+++++++++++++++++&........::.;X:....&.  .+Xx$XX.       .&xxxxxx&xxxxxxxxxxxxx$Xxx$            
                                               :$+++++++++++++++&:.......&&$x:$.....:        :..        Xxxxxxxxxx&x.      .                  
                                                .X++++++++++++x$........$&&&$x....:X.                   XxxxxxxX                              
                                                . .Xxx+++++x$$.........x..:.$.....$.                   .xxxxx.                                
                                            .X+. . .  ..;+$+:...........:x+;....::.                  ..Xxxxx+                                 
                                               ..;+    .   . .    . ...;;;+Xx+$&.                     $xxxx$.                                 
                                              ..X..:+$&$XxX   .. .             .x:.              ..$;;+xxxX.                                  
                  .XX:..                   .&;:..........$.....&;:    .&.    ;$.. ..             .;....xxx+                                   
               .$......$.              .XxxxX..............:x++++$;    :.+x.   .Xx+.            .;.:++:XX:::                                  
               .x.......:$...:.    ..$.....:..&........:....;++++$.:$;.&....:$X:$;..           .&:.XXxxx$..+                                  
                .;:$X+X:..:$.::. :$:......$;.:X.....&XXX;...$x+++;........+&$:...;:X..     .x...::Xxxxx$;..:                                  
                .x+X.::.;+:X.+. .+:.........:$......:$X&..xx++++X:........x&X....$.:.X.      :$..;xxxxX..:x.                                  
                    .:X...Xx    &.........:.&.:......:..$x++++++$................;:...:+. .X:$...Xxxxxx$.                                     
                  .X::..$..x   .;:.........::+:.....;$x++++++++X..................$.....;.....x+$xxxx; X.                                     
                  .X..:+X.$$  &:..x...........:X&$+++++++++++++$..................$.....$......$xxxx$X.;.                                     
                   :;:.:.+&$. .$Xx++x$xxx+++++++++++++++++++++$....+..::+XX+:.....x..::::.....+xxxxX...;.                                     
                        .. ..XX.  .+x++++++++++++++++++++++++$:....x............&::;:........;xxxxx;.                                         
                                .:$+++++++++++++++++++++++++X:.....$............$::++.&:....:$xxxx$.                                          
                             . .x++++++++++++++++++++++++++X.......;.....:X+....&.:;::....::&xxxx$..                                          
                         .. ..;&++++++++++++++++++++++++++X:......x:....$.X   .;$x::.....&;;$xxxX..                                           
                      ...:$&&&&&&$+++++++++++++++++++++++X:.......+.....X:$ . x     X$:...:.  .X..                                            
                     .:&&&&&&&&&&&&&&$x+++++++++++++++++$........X........x;.:&;     .x$+;..  &..                                             
                     :&&&&&&&&&&&&&&&&&&&&&$x++++++++++$........:x............:+.;$ ;  ..+...&..                                              
                    .x&&&&&&&&&&&&&&&&&&&&&&&&&&&&&$X$:.........XX+:..........$:.X::x+. &  .&..                                               
                   ..$&&&&&&&&&&&&&&&&&&&&&&&&&&&&$     .X&$+..:....:.......::...&:. .:&   x:x.                                               
                    .:&&&&&&&&&&&&&&&&&&&&&&&&&+....               .:xXX$X+++:;;;....:$   .: .X..                                             
                     :$&&&&&&&&&&&&&&&&&&&&&&; .                            . .. ;..:++...x.   $..                                            
                     .;&&&&&&&&&&&&&&&&&&&&&&:.....X&X                            .Xx. ..+;;+.$.$..                                           
                     .:&&&&&&&&&&&&&&&&&&&&&&&&&&&&&.       .                      .+  .X...:+...+..                                          
                      .:&&&&&&&&&&&&&&&&&&&&&&&&&&&      . X&..      x.        $.  $  .X.. .:+. .$..                                          
                       :x&&&&&&&&&&&&&&&&&&&&&&&&&. .   .&&&&..      .&;.       $$$   ;... .::x  +..                                          
                       .:&&&&&&&&&&&&&&&&&&&&&&&&&.  .X&&&&&&$.     ..&&&. .   .:X$. .+..  .::X .:..                                          
                        .:&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&.   ..+&&&&&: ...&x ..x..   .:.$X$x...                                         
                         .;&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&$... &&&&&+:...:$   +...   .:;.. $..                                          
                         .:x&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&$:...:..  .;..   ..:X.  x..                                          
                          .:x&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&::..::$.  X...   .:x...::..                                          
                           ...+$&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&+:...::..  +... ...+....x...                                          
                              .     ....;xX&&&&&&&&&&&&&&&&&&&&&&&&&&&&X:. ..:;+..+:.....:&. . X...                                           
                                       ....::x+++++$;+;+$&&&&&&&&&&&&&$:.....:;.  .&.:::$ .   x...                                            
                                         .:+++++++++xx:..........:;+Xx...  ..::x..  ..  .  .$...                                              
                                      ..+xx+++++++++++++Xx....               .:.$..     :x$....                                               
                                   .::x+++++++++++++++++++++X..               ...+&xXXXx...                                                   
                                  .::x+++++++++++++++++++++++$...                                                                             
                                   ..;;$&&&&&$$XXX$XXX$$$$$XX$x..                                                                             
                                     ..:X&&&&&&&$xX&&&&&&x:.                                                                                  
                                            ........   ...                      
                                         
                                                ▐                   
                                        ▗ ▗  ▄▖ ▐ ▗ ▗ ▗ ▗ ▗ ▗▄▄  ▄▖ 
                                        ▝▖▞ ▝ ▐ ▐▗▘ ▝▖▞ ▐ ▐ ▐▐▐ ▐▘▐ 
                                         ▙▌ ▗▀▜ ▐▜   ▙▌ ▐ ▐ ▐▐▐ ▐▀▀ 
                                         ▐  ▝▄▜ ▐ ▚  ▜  ▝▄▜ ▐▐▐ ▝▙▞ 
                                                     ▞              
                                                   ▝▘              
                                        
                                            by Julian Henry
"""

"""Vakyume shard repair: cascading fix strategies (quick-fix, SymPy, brentq, LLM).

When a solver shard fails One-Odd-Out verification, this module generates a
repair prompt and queries the LLM to produce a corrected solver function.
"""

import ast
import importlib
import os
import re
import sys

from sympy import Symbol, solve, sympify

from .config import SHARD_IMPORT_HEADER, TAB, MATH_FUNC_NAMES_WITH_SELF
from .llm import ask_llm, extract_code
from .parser import case_safe_name, find_shard_file


# ---------------------------------------------------------------------------
#  Regex: matches bare ``return expr`` (not already wrapped in a list)
# ---------------------------------------------------------------------------
_BARE_RETURN_RE = re.compile(
    r"return\s+(?!\[)(\w[\w.]*(?:\s*[\+\-\*/]\s*[\w.()]+)*(?:\s*\*\*\s*[\w.()]+)?)\s*$",
    re.MULTILINE,
)


def _postprocess_code(code: str) -> str:
    """Fix common LLM-generated code issues before writing to disk.

    - Replace cmath.func() calls with bare func() (already imported via
      ``from cmath import log, sqrt, exp``)
    - Replace ln() with log()
    - Replace math.func() with bare func() for common functions
    - Replace numpy math calls where cmath equivalents exist
    """
    # cmath.X -> X  (the shard header does `from cmath import log, sqrt, exp`)
    code = re.sub(r"\bcmath\.log\b", "log", code)
    code = re.sub(r"\bcmath\.sqrt\b", "sqrt", code)
    code = re.sub(r"\bcmath\.exp\b", "exp", code)
    code = re.sub(r"\bcmath\.pow\b", "pow", code)
    code = re.sub(r"\bcmath\.phase\b", "phase", code)
    # math.X -> X
    code = re.sub(r"\bmath\.log\b", "log", code)
    code = re.sub(r"\bmath\.sqrt\b", "sqrt", code)
    code = re.sub(r"\bmath\.exp\b", "exp", code)
    code = re.sub(r"\bmath\.pow\b", "pow", code)
    code = re.sub(r"\bmath\.pi\b", "pi", code)
    code = re.sub(r"\bmath\.e\b", "e", code)
    # ln() -> log()  (common LLM mistake)
    code = re.sub(r"\bln\(", "log(", code)
    # Remove stray `import cmath` or `import math` lines that LLMs add
    code = re.sub(r"^\s*import cmath\s*$", "", code, flags=re.MULTILINE)
    code = re.sub(r"^\s*import math\s*$", "", code, flags=re.MULTILINE)
    # Strip comment lines (except pyeqn) to save tokens — phi3 writes too many
    lines = code.splitlines()
    stripped_lines = []
    for line in lines:
        s = line.strip()
        if s.startswith("#") and "[.pyeqn]" not in s:
            continue
        stripped_lines.append(line)
    code = "\n".join(stripped_lines)
    # Convert brentq(f, lo, hi) -> safe_brentq(f) for adaptive bracketing
    code = re.sub(
        r"\bbrentq\s*\(\s*(\w+)\s*,\s*[^,]+\s*,\s*[^)]+\)", r"safe_brentq(\1)", code
    )
    return code


# Alias for backward compatibility — pipeline.py imports _find_shard_file from here
_find_shard_file = find_shard_file


# ---------------------------------------------------------------------------
#  Golden-tuple validation
# ---------------------------------------------------------------------------


def _validate_shard_against_golden(
    shard_path: str,
    method_name: str,
    golden_test_cases: list,
    broken_variant: str,
) -> bool:
    """Test a freshly-written shard against golden-tuple test cases.

    Returns True if the shard passes a majority (>= 50%) of test cases.
    Returns False on load/import failures — broken code should not be accepted.
    """
    if not golden_test_cases:
        return True

    try:
        import importlib.util

        spec = importlib.util.spec_from_file_location("_repair_check", shard_path)
        if spec is None or spec.loader is None:
            print(f"  [Repair] Semantic check FAILED: cannot load shard module")
            return False
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        func = getattr(mod, method_name, None)
        if func is None:
            print(
                f"  [Repair] Semantic check FAILED: function {method_name} not found in shard"
            )
            return False
    except Exception as exc:
        print(f"  [Repair] Semantic check FAILED: import error: {exc}")
        return False

    passes = 0
    for tc in golden_test_cases:
        inputs = tc.get("inputs", {})
        expected = tc.get("expected")
        if expected is None:
            continue
        try:
            got = func(None, **inputs)  # self=None since it's not a real method
            if got is None:
                continue
            if isinstance(got, (list, tuple)):
                for g in got:
                    try:
                        if _values_close(g, expected):
                            passes += 1
                            break
                    except (TypeError, ValueError):
                        continue
            else:
                if _values_close(got, expected):
                    passes += 1
        except Exception:
            continue

    total = len([tc for tc in golden_test_cases if tc.get("expected") is not None])
    if total == 0:
        return True
    threshold = max(1, total // 2)
    if passes < threshold:
        print(
            f"  [Repair] Semantic check FAILED: {passes}/{total} golden tests passed (need >= {threshold})"
        )
        return False
    print(f"  [Repair] Semantic check OK: {passes}/{total} golden tests passed")
    return True


def _values_close(a, b, abs_tol=1e-4, rel_tol=1e-6):
    """Check if two numeric values are close."""
    try:
        a_val = complex(a)
        b_val = complex(b)
    except (TypeError, ValueError):
        return False
    if abs(a_val.imag) < abs_tol:
        a_val = complex(a_val.real, 0)
    if abs(b_val.imag) < abs_tol:
        b_val = complex(b_val.real, 0)
    diff = abs(a_val - b_val)
    mag = max(abs(a_val), abs(b_val), 1e-15)
    return diff < abs_tol or diff / mag < rel_tol


def _try_write_and_validate(
    full_code: str,
    shard_path: str,
    method_name: str,
    golden_test_cases: list,
    broken_variant: str,
    label: str = "",
) -> bool:
    """Write code to a temp file, validate against golden tests, and persist if OK."""
    import tempfile

    try:
        ast.parse(full_code)
    except SyntaxError as exc:
        print(f"  [Repair] {label} code has syntax error: {exc}")
        return False

    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".py", delete=False, dir=os.path.dirname(shard_path)
    ) as tmp:
        tmp.write(full_code)
        tmp_path = tmp.name
    try:
        ok = _validate_shard_against_golden(
            tmp_path, method_name, golden_test_cases or [], broken_variant
        )
    finally:
        try:
            os.unlink(tmp_path)
        except OSError:
            pass

    if ok:
        with open(shard_path, "w") as f:
            f.write(full_code)
        print(f"  [Repair] {label} solve succeeded for {os.path.basename(shard_path)}")
        return True
    return False


# ---------------------------------------------------------------------------
#  SymPy / brentq fallback  (broken into sub-steps)
# ---------------------------------------------------------------------------


def _parse_equation(pyeqn: str, broken_variant: str):
    """Parse an equation string into components for SymPy/brentq solving.

    Returns ``(clean_eq, lhs, rhs, normal_form, eq_vars, other_vars)`` on
    success, or ``None`` if the equation can't be parsed.
    """
    clean_eq = re.sub(
        r"^(?:Solve\s+for\s+\w+\s+in\s+the\s+equation\s+)",
        "",
        pyeqn,
        flags=re.IGNORECASE,
    ).strip()

    if "=" not in clean_eq:
        print(f"  [Repair] SymPy fallback: no '=' found in equation")
        return None

    parts = clean_eq.split("=")
    if len(parts) != 2:
        print(f"  [Repair] SymPy fallback: equation doesn't have exactly 2 sides")
        return None

    lhs = parts[0].strip()
    rhs = parts[1].split("#")[0].strip()
    nf = f"({rhs}) - ({lhs})"
    nf = nf.replace("ln(", "log(").replace("ln (", "log(")

    all_tokens = set(re.findall(r"\b[a-zA-Z_]\w*\b", clean_eq))
    eq_vars = sorted(
        t for t in all_tokens if t not in MATH_FUNC_NAMES_WITH_SELF and not t.isdigit()
    )
    other_vars = [v for v in eq_vars if v != broken_variant]

    return clean_eq, lhs, rhs, nf, eq_vars, other_vars


def _try_algebraic_solve(
    clean_eq: str,
    lhs: str,
    rhs: str,
    normal_form: str,
    eq_vars: list[str],
    broken_variant: str,
    header: str,
    shard_path: str,
    method_name: str,
    golden_test_cases: list,
) -> dict | None:
    """Attempt algebraic solve with SymPy (with 5s timeout).

    Returns ``{"updated": True}`` on success, ``None`` to signal the caller
    should fall through to the next strategy.
    """
    local_dict = {t: Symbol(t) for t in eq_vars}

    try:
        import signal

        def _timeout_handler(signum, frame):
            raise TimeoutError("SymPy solve timed out")

        old_handler = signal.signal(signal.SIGALRM, _timeout_handler)
        signal.alarm(5)
        try:
            expr = sympify(normal_form, locals=local_dict)
            solns = solve(expr, Symbol(broken_variant))
        finally:
            signal.alarm(0)
            signal.signal(signal.SIGALRM, old_handler)

        if not solns:
            return None

        # Generate hybrid function body: evaluate all SymPy roots,
        # filter for real positive ones, fall back to brentq if none found.
        body_lines = [f"{TAB}# [.pyeqn] {clean_eq}", f"{TAB}result = []"]
        for soln in solns:
            body_lines.append(f"{TAB}try:")
            body_lines.append(f"{TAB}{TAB}_v = {soln}")
            body_lines.append(f"{TAB}{TAB}if isinstance(_v, complex):")
            body_lines.append(
                f"{TAB}{TAB}{TAB}if abs(_v.imag) < 1e-6: result.append(_v.real)"
            )
            body_lines.append(f"{TAB}{TAB}else:")
            body_lines.append(f"{TAB}{TAB}{TAB}try:")
            body_lines.append(f"{TAB}{TAB}{TAB}{TAB}_fv = float(_v)")
            body_lines.append(f"{TAB}{TAB}{TAB}{TAB}result.append(_fv)")
            body_lines.append(f"{TAB}{TAB}{TAB}except (TypeError, ValueError): pass")
            body_lines.append(f"{TAB}except Exception: pass")
        # Fall back to brentq if no real roots were found
        body_lines.append(f"{TAB}if not result:")
        body_lines.append(f"{TAB}{TAB}def _residual({broken_variant}):")
        body_lines.append(f"{TAB}{TAB}{TAB}return ({rhs}) - ({lhs})")
        body_lines.append(f"{TAB}{TAB}result.append(safe_brentq(_residual))")
        body_lines.append(f"{TAB}return result")

        code_text = header + "\n" + "\n".join(body_lines) + "\n"
        code_text = _postprocess_code(code_text)
        full_code = SHARD_IMPORT_HEADER + code_text + "\n"

        ok = _try_write_and_validate(
            full_code,
            shard_path,
            method_name,
            golden_test_cases,
            broken_variant,
            label="SymPy algebraic",
        )
        if ok:
            return {"updated": True}
        print(f"  [Repair] SymPy algebraic solution didn't pass golden tests")

    except Exception as exc:
        print(f"  [Repair] SymPy solve failed: {exc}")

    return None


def _find_singularities(clean_eq: str, broken_variant: str, other_vars: list[str]):
    """Analyze the equation for singularity boundaries.

    Returns ``(singularity_params, literal_singularities)`` where each element
    is a list of strings representing the singularity expressions.
    """
    singularity_params = []
    for pat in [
        rf"\({re.escape(broken_variant)}\s*-\s*(\w+)\)",
        rf"\((\w+)\s*-\s*{re.escape(broken_variant)}\)",
    ]:
        for m in re.finditer(pat, clean_eq):
            p = m.group(1)
            if p in other_vars:
                singularity_params.append(p)

    literal_singularities = []
    for pat in [
        rf"\({re.escape(broken_variant)}\s*-\s*(\d+(?:\.\d+)?)\)",
        rf"\((\d+(?:\.\d+)?)\s*-\s*{re.escape(broken_variant)}\)",
    ]:
        for m in re.finditer(pat, clean_eq):
            literal_singularities.append(m.group(1))

    if re.search(rf"/\s*{re.escape(broken_variant)}\b", clean_eq):
        singularity_params.append("__zero__")

    singularity_params = list(dict.fromkeys(singularity_params))  # dedupe
    return singularity_params, literal_singularities


def _build_brentq_body(
    clean_eq: str,
    lhs: str,
    rhs: str,
    broken_variant: str,
    singularity_params: list[str],
    literal_singularities: list[str],
) -> list[str]:
    """Generate the function body lines for a brentq-based numerical solver.

    Returns the list of indented body lines (excluding the ``def`` header).
    """
    body_lines = [
        f"{TAB}# [.pyeqn] {clean_eq}",
        f"{TAB}def _residual({broken_variant}):",
        f"{TAB}{TAB}return ({rhs}) - ({lhs})",
    ]

    if not singularity_params and not literal_singularities:
        body_lines.append(f"{TAB}return [safe_brentq(_residual)]")
        return body_lines

    # Parameter-aware bracketing around singularities
    sing_exprs = []
    for p in singularity_params:
        sing_exprs.append("0" if p == "__zero__" else p)
    for lit in literal_singularities:
        sing_exprs.append(lit)
    if not sing_exprs:
        sing_exprs.append("0")

    body_lines.append(f"{TAB}from scipy.optimize import brentq as _brentq")
    body_lines.append(f"{TAB}import math as _math")
    body_lines.append(f"{TAB}def _rf(x):")
    body_lines.append(f"{TAB}{TAB}v = _residual(x)")
    body_lines.append(
        f"{TAB}{TAB}return v.real if isinstance(v, complex) else float(v)"
    )
    body_lines.append(f"{TAB}_sings = sorted(set([{', '.join(sing_exprs)}]))")
    body_lines.append(f"{TAB}_intervals = []")
    # Interval below smallest singularity
    body_lines.append(f"{TAB}if _sings[0] > 1e-12:")
    body_lines.append(f"{TAB}{TAB}_e0 = min(0.01, _sings[0] * 0.001)")
    body_lines.append(f"{TAB}{TAB}_intervals.append((1e-12, _sings[0] - _e0))")
    # Intervals between consecutive singularities
    body_lines.append(f"{TAB}for _i in range(len(_sings) - 1):")
    body_lines.append(f"{TAB}{TAB}_gap = _sings[_i+1] - _sings[_i]")
    body_lines.append(f"{TAB}{TAB}_eg = min(0.01, _gap * 0.001)")
    body_lines.append(
        f"{TAB}{TAB}if _gap > 2 * _eg: _intervals.append((_sings[_i] + _eg, _sings[_i+1] - _eg))"
    )
    # Interval above largest singularity
    body_lines.append(
        f"{TAB}_top = _sings[-1] + min(0.01, max(abs(_sings[-1]) * 0.001, 1e-10))"
    )
    body_lines.append(
        f"{TAB}for _hi in [_top + 1, _top + 10, _top * 100, _top + 1000, 1e6]:"
    )
    body_lines.append(f"{TAB}{TAB}_intervals.append((_top, _hi))")
    # Search all intervals with subdivision
    body_lines.append(f"{TAB}_roots = []")
    body_lines.append(f"{TAB}for _lo, _hi in _intervals:")
    body_lines.append(f"{TAB}{TAB}if _lo >= _hi: continue")
    body_lines.append(f"{TAB}{TAB}_N = 50")
    body_lines.append(f"{TAB}{TAB}_step = (_hi - _lo) / _N")
    body_lines.append(f"{TAB}{TAB}for _j in range(_N):")
    body_lines.append(f"{TAB}{TAB}{TAB}_a = _lo + _j * _step")
    body_lines.append(f"{TAB}{TAB}{TAB}_b = _lo + (_j + 1) * _step")
    body_lines.append(f"{TAB}{TAB}{TAB}try:")
    body_lines.append(f"{TAB}{TAB}{TAB}{TAB}_fa = _rf(_a)")
    body_lines.append(f"{TAB}{TAB}{TAB}{TAB}_fb = _rf(_b)")
    body_lines.append(
        f"{TAB}{TAB}{TAB}{TAB}if _math.isfinite(_fa) and _math.isfinite(_fb) and _fa * _fb < 0:"
    )
    body_lines.append(f"{TAB}{TAB}{TAB}{TAB}{TAB}_roots.append(_brentq(_rf, _a, _b))")
    body_lines.append(f"{TAB}{TAB}{TAB}except Exception:")
    body_lines.append(f"{TAB}{TAB}{TAB}{TAB}continue")
    body_lines.append(f"{TAB}if _roots:")
    body_lines.append(f"{TAB}{TAB}_pos = [r for r in _roots if r > 0]")
    body_lines.append(
        f"{TAB}{TAB}return list(set(round(r, 10) for r in (_pos if _pos else _roots)))"
    )
    body_lines.append(f"{TAB}return [safe_brentq(_residual)]")

    return body_lines


def _try_brentq_solve(
    clean_eq: str,
    lhs: str,
    rhs: str,
    broken_variant: str,
    other_vars: list[str],
    header: str,
    shard_path: str,
    method_name: str,
    golden_test_cases: list,
) -> dict | None:
    """Attempt numerical solve with safe_brentq (singularity-aware).

    Returns ``{"updated": True}`` on success, ``None`` to fall through.
    """
    print(f"  [Repair] SymPy fallback: trying safe_brentq wrapper for {broken_variant}")

    singularity_params, literal_singularities = _find_singularities(
        clean_eq, broken_variant, other_vars
    )

    try:
        body_lines = _build_brentq_body(
            clean_eq,
            lhs,
            rhs,
            broken_variant,
            singularity_params,
            literal_singularities,
        )
        code_text = header + "\n" + "\n".join(body_lines) + "\n"
        code_text = _postprocess_code(code_text)
        full_code = SHARD_IMPORT_HEADER + code_text + "\n"

        ast.parse(full_code)

        ok = _try_write_and_validate(
            full_code,
            shard_path,
            method_name,
            golden_test_cases,
            broken_variant,
            label="safe_brentq numerical",
        )
        if ok:
            return {"updated": True}
        print(f"  [Repair] safe_brentq solution didn't pass golden tests")
    except SyntaxError as exc:
        print(f"  [Repair] safe_brentq wrapper had syntax error: {exc}")
    except Exception as exc:
        print(f"  [Repair] safe_brentq wrapper failed: {exc}")

    return None


def _sympy_fallback(
    pyeqn: str,
    broken_variant: str,
    eqn_suffix: str,
    header: str,
    shard_path: str,
    golden_test_cases: list,
) -> dict:
    """Last-resort fallback: solve algebraically with SymPy or numerically with safe_brentq.

    Returns ``{"updated": True}`` if a valid shard was generated,
    ``{"updated": False}`` otherwise.
    """
    print(f"  [Repair] SymPy fallback: attempting algebraic solve for {broken_variant}")

    method_name = f"eqn_{eqn_suffix}__{broken_variant}"

    # Don't overwrite a shard that already passes golden tests
    if os.path.exists(shard_path) and golden_test_cases:
        if _validate_shard_against_golden(
            shard_path, method_name, golden_test_cases, broken_variant
        ):
            print(
                f"  [Repair] Existing shard already passes golden tests — skipping SymPy fallback"
            )
            return {"updated": True}

    parsed = _parse_equation(pyeqn, broken_variant)
    if parsed is None:
        return {"updated": False}
    clean_eq, lhs, rhs, nf, eq_vars, other_vars = parsed

    # Attempt 1: algebraic solve
    result = _try_algebraic_solve(
        clean_eq,
        lhs,
        rhs,
        nf,
        eq_vars,
        broken_variant,
        header,
        shard_path,
        method_name,
        golden_test_cases,
    )
    if result is not None:
        return result

    # Attempt 2: numerical solve with brentq
    result = _try_brentq_solve(
        clean_eq,
        lhs,
        rhs,
        broken_variant,
        other_vars,
        header,
        shard_path,
        method_name,
        golden_test_cases,
    )
    if result is not None:
        return result

    return {"updated": False}


# ---------------------------------------------------------------------------
#  attempt_repair_shard  (broken into sub-steps)
# ---------------------------------------------------------------------------


def _extract_shard_context(ctx, family_name: str, broken_variant: str):
    """Read the broken shard and extract its pyeqn, header, and path.

    Returns ``(shard_path, shard_file, shard_code, pyeqn, header, eqn_suffix)``
    or ``None`` if the shard can't be found.
    """
    family_dir = os.path.join(ctx.shards_dir, family_name)
    eqn_suffix = family_name.split("_eqn_")[1]
    shard_path = _find_shard_file(family_dir, eqn_suffix, broken_variant)

    if not shard_path:
        print(f"  [Repair] Shard for '{broken_variant}' not found in {family_name}")
        return None

    shard_file = os.path.basename(shard_path)

    with open(shard_path, "r") as f:
        shard_code = f.read()

    pyeqn_match = re.search(r"# \[\.pyeqn\] (.*)", shard_code)
    pyeqn = pyeqn_match.group(1).strip() if pyeqn_match else ""
    pyeqn = re.sub(
        r"^(?:Solve\s+for\s+\w+\s+in\s+the\s+equation\s+)",
        "",
        pyeqn,
        flags=re.IGNORECASE,
    ).strip()

    # Extract the function header from the broken shard
    header_match = re.search(
        r"(def eqn_.*?\*\*kwargs\s*,?\s*\)):", shard_code, re.DOTALL
    )
    header = (
        re.sub(r"\s+", " ", header_match.group(1)).strip() + ":" if header_match else ""
    )

    # Validate header: target var must NOT be a parameter, other vars must be present
    header = _validate_or_regen_header(header, pyeqn, broken_variant, eqn_suffix)

    return shard_path, shard_file, shard_code, pyeqn, header, eqn_suffix


def _validate_or_regen_header(
    header: str, pyeqn: str, broken_variant: str, eqn_suffix: str
) -> str:
    """Ensure the function header is correct, regenerating it if needed."""
    if not header or not pyeqn:
        return header

    all_tokens = set(re.findall(r"\b[a-zA-Z_]\w*\b", pyeqn))
    eq_vars = sorted(
        t for t in all_tokens if t not in MATH_FUNC_NAMES_WITH_SELF and not t.isdigit()
    )
    other_vars = [v for v in eq_vars if v != broken_variant]

    param_section = header.split("(", 1)[1] if "(" in header else ""
    param_tokens = set(re.findall(r"\b[a-zA-Z_]\w*\b", param_section))
    param_tokens -= {"self", "kwargs", "float", "int"}

    needs_regen = broken_variant in param_tokens
    if not needs_regen:
        for ov in other_vars:
            if ov not in param_tokens:
                needs_regen = True
                break

    if needs_regen:
        func_name = f"eqn_{eqn_suffix}__{broken_variant}"
        params = ", ".join(f"{v}: float" for v in other_vars)
        header = f"def {func_name}(self, {params}, **kwargs):"
        print(f"  [Repair] Regenerated header: {header}")

    return header


def _try_quick_fix(
    shard_code: str,
    shard_path: str,
    method_name: str,
    golden_test_cases: list,
    broken_variant: str,
    shard_file: str,
) -> dict | None:
    """Try to fix the existing shard by applying postprocessing + return wrapping.

    Returns ``{"updated": True}`` on success, ``None`` to fall through.
    """
    qf_code = shard_code
    def_idx = qf_code.find("\ndef ")
    if def_idx == -1:
        def_idx = qf_code.find("def ")
    if def_idx < 0:
        return None

    qf_body = qf_code[def_idx:].lstrip("\n")
    qf_body = _BARE_RETURN_RE.sub(r"return [\1]", qf_body)
    qf_body = _postprocess_code(qf_body)
    qf_full = SHARD_IMPORT_HEADER + qf_body + "\n"

    if qf_full == shard_code:
        return None  # postprocessing changed nothing

    try:
        ast.parse(qf_full)
    except SyntaxError:
        print(f"  [Repair] Quick-fix pass had syntax errors — proceeding to LLM")
        return None

    ok = _try_write_and_validate(
        qf_full,
        shard_path,
        method_name,
        golden_test_cases or [],
        broken_variant,
        label="Quick-fix",
    )
    if ok:
        print(f"  [Repair] Quick-fix pass succeeded for {shard_file} — skipped LLM")
        return {"updated": True}

    print(f"  [Repair] Quick-fix pass did not pass golden tests — proceeding to LLM")
    return None


def _sanitize_llm_code(
    raw_code: str,
    method_name: str,
    header: str,
    pyeqn: str,
) -> str:
    """Clean up LLM-generated code: force correct header, strip imports, wrap returns."""
    code_text = extract_code(raw_code, target_name=method_name)
    if not code_text.strip():
        code_text = extract_code(raw_code)
    if not code_text.strip():
        return ""

    # Force the correct header — don't trust what the LLM wrote
    any_def = re.search(r"def\s+\w+\s*\(.*?\)\s*(?:->.*?)?:", code_text, re.DOTALL)
    if any_def and header:
        code_text = code_text.replace(any_def.group(0), header)
    elif any_def:
        code_text = code_text.replace(
            any_def.group(0),
            re.sub(r"def\s+\w+", f"def {method_name}", any_def.group(0)),
        )

    # Strip stray imports
    body_lines = []
    for line in code_text.splitlines():
        s = line.strip()
        if s.startswith("import ") or s.startswith("from "):
            continue
        body_lines.append(line)
    code_text = "\n".join(body_lines)

    # Inject pyeqn comment if missing
    if "# [.pyeqn]" not in code_text and pyeqn:
        code_text = re.sub(
            r"(def\s+\w+\(.*?\):)",
            rf"\1\n    # [.pyeqn] {pyeqn}",
            code_text,
            count=1,
        )

    # Wrap bare scalar returns in a list
    code_text = _BARE_RETURN_RE.sub(r"return [\1]", code_text)
    code_text = _postprocess_code(code_text)

    return code_text


def _build_repair_prompt(
    pyeqn: str, broken_variant: str, golden_test_cases: list, header: str
):
    """Build the initial LLM prompt for repairing a shard."""
    user_prompt = f"{pyeqn}\nSolve for {broken_variant}.\n"
    if golden_test_cases:
        tc = golden_test_cases[0]
        inputs_str = ", ".join(
            f"{k}={v}" for k, v in sorted(tc.get("inputs", {}).items())
        )
        user_prompt += (
            f"Test: {inputs_str} -> {broken_variant}={tc.get('expected', '?')}\n"
        )
    user_prompt += f"\n{header}\n"
    return user_prompt


_LLM_SYSTEM_PROMPT = (
    "Write a Python function. Output ONLY code. No markdown. No comments.\n"
    "Use log/sqrt/exp from cmath. Return [value]. Keep the signature."
)

MAX_SYNTAX_RETRIES = 3
MAX_SEMANTIC_RETRIES = 3


def _run_llm_repair_loop(
    pyeqn: str,
    broken_variant: str,
    header: str,
    shard_path: str,
    shard_file: str,
    eqn_suffix: str,
    golden_test_cases: list,
) -> dict:
    """Run the LLM repair loop with syntax and semantic retries.

    Returns ``{"updated": True}`` on success, ``{"updated": False}`` otherwise.
    """
    method_name = f"eqn_{eqn_suffix}__{broken_variant}"
    user_prompt = _build_repair_prompt(pyeqn, broken_variant, golden_test_cases, header)
    raw = ask_llm(_LLM_SYSTEM_PROMPT, user_prompt, stream=False)

    if not raw:
        print(f"  [Repair] LLM returned empty response")
        return {"updated": False}

    syntax_failures = 0
    semantic_failures = 0
    max_total_attempts = MAX_SYNTAX_RETRIES + MAX_SEMANTIC_RETRIES

    for attempt in range(1, max_total_attempts + 1):
        code_text = _sanitize_llm_code(raw, method_name, header, pyeqn)
        if not code_text:
            print(f"  [Repair] extract_code returned empty")
            return {"updated": False}

        full_code = SHARD_IMPORT_HEADER + code_text + "\n"

        try:
            ast.parse(full_code)

            # Semantic validation against golden tuples
            ok = _try_write_and_validate(
                full_code,
                shard_path,
                method_name,
                golden_test_cases or [],
                broken_variant,
                label="LLM",
            )

            if not ok:
                semantic_failures += 1
                if semantic_failures < MAX_SEMANTIC_RETRIES:
                    print(
                        f"  [Repair] Semantically wrong (semantic attempt "
                        f"{semantic_failures}/{MAX_SEMANTIC_RETRIES}), retrying..."
                    )
                    retry_prompt = _build_repair_prompt(
                        pyeqn, broken_variant, golden_test_cases, header
                    )
                    retry_prompt = (
                        retry_prompt.rstrip("\n")
                        + f"\nWrong code:\n{code_text}\nFix it.\n"
                    )
                    raw = ask_llm(_LLM_SYSTEM_PROMPT, retry_prompt, stream=False)
                    if not raw:
                        print(f"  [Repair] LLM returned empty on semantic-fix retry")
                        return {"updated": False}
                    continue
                else:
                    print(
                        f"  [Repair] Semantic validation failed after "
                        f"{MAX_SEMANTIC_RETRIES} semantic attempts"
                    )
                    return {"updated": False}

            with open(shard_path, "w") as f:
                f.write(full_code)
            print(f"  [Repair] Successfully wrote LLM-repaired shard: {shard_file}")
            return {"updated": True}

        except SyntaxError as e:
            syntax_failures += 1
            if syntax_failures < MAX_SYNTAX_RETRIES:
                print(
                    f"  [Repair] Syntax error (syntax attempt "
                    f"{syntax_failures}/{MAX_SYNTAX_RETRIES}): {e}"
                )
                print(f"  [Repair] Feeding code back to LLM for correction...")
                fix_prompt = (
                    f"{pyeqn}\nSolve for {broken_variant}.\n"
                    f"Syntax error: {e}\nFix it. Output ONLY the function.\n\n{header}\n"
                )
                raw = ask_llm(_LLM_SYSTEM_PROMPT, fix_prompt, stream=False)
                if not raw:
                    print(f"  [Repair] LLM returned empty on syntax-fix retry")
                    return {"updated": False}
            else:
                print(
                    f"  [Repair] LLM produced invalid syntax after "
                    f"{MAX_SYNTAX_RETRIES} syntax attempts"
                )
                return {"updated": False}

    return {"updated": False}


# ---------------------------------------------------------------------------
#  Main entry point
# ---------------------------------------------------------------------------


def attempt_repair_shard(
    ctx,
    family_name: str,
    broken_variant: str,
    trusted_variants: list,
    scores: dict,
    mismatches: dict,
    error: str = None,
    golden_test_cases: list = None,
):
    """Repair a single shard file using cascading strategies.

    Strategy order:
      1. Quick-fix (postprocessing + return wrapping)
      2. SymPy algebraic solve
      3. Numerical brentq solve
      4. LLM-assisted code generation with retry loop
    """
    context = _extract_shard_context(ctx, family_name, broken_variant)
    if context is None:
        return {"updated": False}

    shard_path, shard_file, shard_code, pyeqn, header, eqn_suffix = context
    method_name = f"eqn_{eqn_suffix}__{broken_variant}"

    print(f"\n |- Repairing shard: {shard_file} in family {family_name}")

    # Strategy 1: Quick-fix pass
    result = _try_quick_fix(
        shard_code,
        shard_path,
        method_name,
        golden_test_cases,
        broken_variant,
        shard_file,
    )
    if result is not None:
        return result

    # Strategy 2 & 3: SymPy algebraic / brentq pass
    sympy_result = _sympy_fallback(
        pyeqn,
        broken_variant,
        eqn_suffix,
        header,
        shard_path,
        golden_test_cases or [],
    )
    if sympy_result.get("updated"):
        return sympy_result

    # Strategy 4: LLM repair loop
    return _run_llm_repair_loop(
        pyeqn,
        broken_variant,
        header,
        shard_path,
        shard_file,
        eqn_suffix,
        golden_test_cases or [],
    )
