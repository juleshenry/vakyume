...
"""Derivation-scaffold generation for LLM-assisted equation solving.

When SymPy cannot isolate a variable algebraically, this module builds a
partially-completed code scaffold that guides the LLM (Phi-3) to fill in the
final expression.  Each pattern handler recognises a specific algebraic form
and returns a scaffold string, or ``None`` to fall through to the next handler.
"""
...
def _scaffold_ratio_power_target_in_denominator(
    ...
    """Branch 3: ``(A / target) ** n``."""
    m = re.search(
        r"\(\s*(\w+)\s*/\s*" + re.escape(target_var) + r"\s*\)" r"\s*\*\*\s*([\d.]+)",
        eq,
    )
    if not m:
        return None
...
def _scaffold_ratio_power_solve_numerator(
    ...
    m = re.search(
        r"\(\s*" + re.escape(target_var) + r"\s*/\s*(\w+)\s*\)" r"\s*\*\*\s*([\d.]+)",
        eq,
    )
    if not m:
        return None
...
# ---------------------------------------------------------------------------
#  Handler registry and main dispatcher
# ---------------------------------------------------------------------------
...
def generate_derivation_scaffold(pyeqn: str, target_var: str, header: str) -> str:
