...
def _collect_class_methods(
    ...
    """
    Scan *shards_dir* and collect per-class method dictionaries.

    Returns (class_methods, class_order) where:
      class_methods: { class_name: { method_name: method_source } }
      class_order:   { class_name: first_seen_family_dir_name }
    """
...
def _build_init_py(class_names: list[str]) -> str:
    """Build an __init__.py that imports all chapter classes."""
    lines = ['"""Auto-generated package — one module per chapter."""', ""]
    for class_name in class_names:
        module_name = class_name.lower()
        lines.append(f"from .{module_name} import {class_name}")
    lines.append("")
    # Build __all__
    all_names = ", ".join(f'"{c}"' for c in class_names)
...
def reconstruct_cli(project_dir: str, output: str | None = None, stdout: bool = False):
    ...
    for class_name in sorted_classes:
        ...
        if not methods:
            continue
        module_src = _build_class_module(class_name, methods)
        module_file = os.path.join(py_dir, f"{class_name.lower()}.py")
        with open(module_file, "w") as f:
            f.write(module_src)

    # Write __init__.py
    init_src = _build_init_py(sorted_classes)
