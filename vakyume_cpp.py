...
def main():
    certified_file = "/Users/enrique/Desktop/fun_repos/vakyume/vakyume_certified.py"
    ...
    print(f"Converting {len(tasks_to_run)} functions...")
    ...
    with open("vakyume.cpp", "w") as f:
        f.write("\n".join(cpp_output))

    print("Compilation...")
    # Using g++ as it's the standard for C++ (gcc can also work but g++ is safer)
    compile_res = subprocess.run(
        ["g++", "vakyume.cpp", "-o", "vakyume_test"], capture_output=True, text=True
    )
    if compile_res.returncode != 0:
        print("Compilation FAILED:")
        print(compile_res.stderr)
    else:
        print("Compilation SUCCESS. Running tests...")
        run_res = subprocess.run(["./vakyume_test"], capture_output=True, text=True)
